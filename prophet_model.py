...
def train_prophet_model(data, ticker='STOCK', forecast_horizon='1y', timestamp=None):
    """
    ...
    # ----- Generate Report -----
    # (Ensure your `create_full_report` in report_generator.py is updated to handle the new aggregation format.)
    report_path = create_full_report(
        ticker=ticker,
        actual_data=agg_actual,
        forecast_data=agg_forecast,
        historical_data=historical_data,
        ts=timestamp if timestamp is not None else int(pd.Timestamp.now().timestamp()),
        aggregation=agg_format
    )
